...
class TaxController extends AbstractController
{
    ...
    {
        $qb = $em->getRepository(Tva::class)->findBy([], ['createdAt' => 'desc']);
        ...
        EventDispatcherInterface $dispatcher)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_admin_tax_index');
        }
    ...
    {
        $tax = $em->getRepository(Tva::class)->find($id);
        $form = $this->createForm(TaxType::class, $tax);
    ...
    )
    {
        ...
        if ($request->getMethod() == 'DELETE') {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                foreach ($ids as $id) {
                    $tax = $em->getRepository(ShipmentMethod::class)->find($id);
    ...
    private function deleteForm(Tva $tva)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('app_admin_tax_delete', ['id' => $tva->getId()]))
            ->setMethod('DELETE')
            ->getForm();
    }
